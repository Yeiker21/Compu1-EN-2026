...
template<> struct Show<int> { static std::string show(int x) { return std::to_string(x); } };
template <> struct Show<double> { static std::string show(double x) { return std::to_string(x); } };
template<> struct Show<std::string> { static std::string show(std::string x) { return x; } };
template<> struct Show<bool> { static std::string show(bool x) { return x ? "true" : "false"; } };
...
public:
    ...
    List<t> cons(t x) const { List<t> xs(*this); xs._inner.push_back(x); return xs; }
    List<t> tail() const { List<t> xs(*this); if (!xs._inner.empty()) xs._inner.pop_back(); return xs; }
    t head() const { return _inner.back(); }
    bool isEmpty() const { return _inner.empty(); }
};
...
struct Eq<List<t>> {
    static bool eq(List<t> xs, List<t> ys) {
        if (xs.isEmpty() && ys.isEmpty()) return true;
        if (xs.isEmpty() || ys.isEmpty()) return false;
        if (!Eq<t>::eq(xs.head(), ys.head())) return false;
        return eq(xs.tail(), ys.tail());
    }
};

template <typename t> requires ShowImplementation<t>
std::string show(const t& x) { return Show<t>::show(x); }

template <typename t> requires EqImplementation<t>
bool operator==(const List<t>& xs, const List<t>& ys) { return Eq<List<t>>::eq(xs, ys); }
...
template <typename b, typename a, typename f_a_b>
...
List<b> mapRec(f_a_b f, List<a> const& xs) {
    ...
    return mapRec<b>(f, xs.tail()).cons(f(xs.head()));
}

template <typename b, typename a, typename f_a_b>
List<b> mapReduce(f_a_b f, List<a> const& xs) {
    return reduceLeft([f](List<b> acc, a val) { return acc.cons(f(val)); }, List<b>(), xs);
}
...
List<a> filterMap(f_a_b f, List<a> const& xs) {
    auto m = map<List<a>>([f](a v) { 
        List<a> l; return f(v) ? l.cons(v) : l; 
    }, xs);
    return concat(m);
}
...
List<a> filterReduce(f_a_b f, List<a> const& xs) {
    return reduceLeft([f](List<a> acc, a v) { return f(v) ? acc.cons(v) : acc; }, List<a>(), xs);
}
...
List<t> dropEveryN(int n, List<t> const& xs) {
    ...
    return map<t>([](std::tuple<t, int> p){ return std::get<0>(p); }, 
           filter([n](std::tuple<t, int> p){ return std::get<1>(p) % n != 0; }, zip(xs, idx)));
}
...
List<t> rotateByN(List<t> const& xs, int n) {
    ...
    auto srt = sortBy(zip(xs, ids), [sh, s](std::tuple<t, int> a, std::tuple<t, int> b) {
        return ((std::get<1>(a)-sh+s)%s) <= ((std::get<1>(b)-sh+s)%s);
    });
    return map<t>([](std::tuple<t, int> p){ return std::get<0>(p); }, srt);
}
